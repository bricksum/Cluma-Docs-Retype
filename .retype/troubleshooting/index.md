...
### SSH · VS Code · Jupyter 비밀번호가 뭔가요?

컨테이너마다 자동으로 만들어지는 **24자 공통 비밀번호** 입니다. 세 가지 모두 같은 비밀번호로 들어갑니다.

- 컨테이너 상세 화면의 **SSH 접속 정보** 영역에서 복사
- 이 비밀번호는 컨테이너 한정 — Cluma 웹 로그인용 비밀번호와는 다릅니다

!!!note SSH 공개키 등록(SSH Key)은 현재 비활성
v1.1에서 SSH 공개키 등록 UI는 일시적으로 숨겨져 있습니다. 그 동안에는 자동 발급 비밀번호로 접속합니다.
!!!
