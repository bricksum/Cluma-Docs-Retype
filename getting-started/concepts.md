...
| 공유 메모리 (Shared Memory) | `/dev/shm` 크기. GPU 여러 개로 학습할 때 자주 필요 |
| SSH Key | 비밀번호 없이 접속하기 위한 공개키 |
...
- 템플릿이 기억하던 스토리지가 사라졌다면, 그 템플릿으로는 새 컨테이너를 만들 수 없습니다. 화면이 이유를 알려줍니다.
...
| 공유 메모리 | 컨테이너 안 `/dev/shm` 크기. GPU 여러 개 학습 시 자주 늘림 |
